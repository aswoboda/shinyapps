...
While the firm chooses the level of production, the market price is determined by the demand side of the market. Higher demand will lead to a higher equilibrium price, holding the quantity constant. A linear demand curve is determined by two values, the demand curve `intercept` and `slope`. <!-- Can I make an app such that I can choose to either calculate P(Q) or Q(P)? -->
...
It would be amazing to have a conditional panel (like a "next button?") that could add functionality to the shiny app after someone says they're ready, or answers a question...

## Total Revenue

A firm that can influence the market price by increasing or decreasing the market quantity is also interested on the production decision's effect on revenue. First, remember that 
...
Try different levels of q and notice what happens to to the Total Revenue in the market. You should notice a pattern that the highest Total Revenue values tend to occur in the "middle" of the demand curve, while small q (but high P) or large q (but low P) combinations tend to create less revenue. This is not a coincidence. In fact, Total Revenue follows a very specific pattern for linear demand curves.

$$TR = P*Q$$
but we know that in this market given our demand curve $P(Q) = (a-b*Q)$,
so 
