...
## GOAL: Introduce undergraduates to the concept of Marginal Revenue for a firm facing a linear demand curve. 

Throughout this section, we assume the fundamental choice variable for a firm is the quantity of production, $q$. 

## Linear Demand Curve

While the firm chooses the level of production, the market price is determined by the demand side of the market. Higher demand will lead to a higher equilibrium price, holding the quantity constant. A linear demand curve is determined by two values, the demand curve intercept, `a` and slope `b`. <!-- Can I make an app such that I can choose to either calculate P(Q) or Q(P)? It would be amazing to have a conditional panel (like a "next button?") that could add functionality to the shiny app after someone says they're ready, or answers a question...-->
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput(inputId = "a",
               label = "Demand Curve Intercept: a",
               value = 100),
      numericInput(inputId = "b",
                   label = "Demand Curve Slope: b",
                   value = 2),
      uiOutput("int_slider"),
...
A single firm in the market can influence the market price by increasing or decreasing the quantity in the market. Such a firm will be interested in the relationship between quantity and total revenue. 

Remember that Total Revenue is calculated as price times quantity.
...
Try different levels of q and notice what happens to the Total Revenue in the market. You should notice a pattern that the highest Total Revenue values tend to occur in the "middle" of the demand curve, while small q (but high P) or large q (but low P) combinations tend to create less revenue. This is not a coincidence. 

## 
Total Revenue follows a very specific pattern for linear demand curves.
...
## 
We can see the Marginal Revenue on our Total Revenue graph as the slope of the Total Revenue graph (the change in Total Revenue for a one unit change in q). 

## 
While the linear demand curve has a constant slope, our Total Revenue curve does not. The slope depends on the value of q (and our underlying demand curve). For small values of q the slope of TR is positive. That is, producing one more unit leads to an *increase* in Total Revenue. Marginal Revenue is positive. For "large" values of q (relative to the demand curve), the slope of TR is negative. Producing extra q leads to a *lower* level of Total Revenue. Marginal Revenue is negative. We can also see that for "medium" levels of q, Total Revenue is pretty similar no matter the value. In fact, there is even one level of q such that the Marginal Revenue is exactly 0 (increasing q lead to the same level of Total Revenue).
