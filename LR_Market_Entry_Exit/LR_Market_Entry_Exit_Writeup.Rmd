...
ui <- fluidPage(
  fluidRow(
    column(6, 
           ...
           column(6,
                  h3("Curves to plot"),
                  checkboxInput("show_Demand", "Plot Demand Curve"),
                  checkboxInput(inputId = "show_profit",
                    label = "Show Profits",
                    value = FALSE))
  ),
...
server <- function(input, output) {
  ...
  output$market_plot <- renderPlot({ 
    plot(1, 1, type = "n", las = 1,
         ...
         main = "Market")
    abline(input$a, -1*input$b, lwd = 2, col = colDemand)
    abline(input$c, input$d/input$n, lwd = 2, col = colSupply)
  ...
  output$marginal_plot <- renderPlot({
    ...
    abline(h = invDemand(Qstar(input)), lty = 3)
    abline(input$c, input$d, lwd = 2, col = colMC)
  ...
  output$total_plot <- renderPlot({
    ...
    plot(1, 1, type = "n", las = 1,
         ...
         main = "Total Analysis")
    abline(0, invDemand(Qstar(input)), lwd = 2, col = colTR)
    curve(TC, from = -1, add = TRUE, lwd = 2, col = colTC)
